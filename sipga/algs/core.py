...
from sipga.common.online_mean_std import OnlineMeanStd
from sipga.algs.vtrace import calculate_v_trace
...
def get_optimizer(opt: str,
    ...
    if opt == 'SGD':  # does not use eps in the c
        return optimizer(module.parameters(), lr=lr)
    else:
        return optimizer(module.parameters(), lr=lr, eps=eps)
...
def get_registered_actor_fn(actor_type: str, distribution_type: str):
    ...
    assert actor_fn in registered_actors, f'Did not find: {actor_fn} in registered actors.'
...
class MLPGaussianActor(Actor):
    ...
    def set_log_std(self, frac):
        ...
        assert 0 <= frac <= 1
        new_stddev = 0.3 * frac + 0.2  # linear anneal from 0.5 to 0.2
...
class MLPCritic(nn.Module):
    ...
    def __init__(self, obs_dim, hidden_sizes, activation, shared=None):
        super().__init__()
        if shared is None:
            self.v_net = build_mlp_network([obs_dim] + list(hidden_sizes) + [1],
                                           activation=activation)
        else:  # use shared layers
            value_head = nn.Linear(hidden_sizes[-1], 1)
            self.v_net = nn.Sequential(shared, value_head, nn.Identity())
...
class Buffer:
    def __init__(self,
        ...
        self.subtract_advantage_mean = subtract_advantage_mean
        self.ptr = 0
    ...
    def calculate_adv_and_value_targets(self, vals, rews, lam=None):
        ...
        if self.adv_estimation_method == 'gae':
            ...
            obs = self.actor_critic.obs_oms(self.obs_buf[path_slice], clip=False) \
    ...
    def finish_path(self, last_val=0):
        ...
        if self.use_scaled_rewards:
            ...
            for i, ret in enumerate(discounted_ret):
                ...
                self.actor_critic.ret_oms.update(ret)
                # TODO: Mentioned by reviewer #7, Q2
                # now scale...
                rews[i] = self.actor_critic.ret_oms(rews[i],
                                                    subtract_mean=False,
                                                    clip=True)
