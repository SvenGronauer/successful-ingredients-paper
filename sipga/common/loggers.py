...
def setup_logger_kwargs(exp_name='vpg',
    ...
    if seed is not None:
        subfolder = '_'.join(['seed', str(seed).zfill(4)])
...
class Logger:
    ...
    def __init__(self,
        ...
        print(colorize("Logging data to %s" % self.output_file.name,
                       'cyan', bold=True))
        ...
        # Write hyper-parameters to Tensor Board
    ...
    def debug(self, msg, color='yellow'):
        ...
        print(colorize(msg, color, bold=False)) if self.debug else None
    ...
    def log_tabular(self, key, val):
        ...
        # self._run.log_scalar(key, value=float(val))
    ...
    def save_config(self, config):
        ...
        # filter_values_in_dict(config)

        # if self.summary_writer:
        #     metrics = {'acc': 10}
        #     self.summary_writer.add_hparams(filter_values_in_dict(config),
        #                                     metric_dict=metrics)
    ...
    def dump_tabular(self):
        ...
        print("-" * n_slashes) if self.verbose else None
        for key in self.log_headers:
            val = self.log_current_row.get(key, "")
            valstr = "%8.3g" % val if hasattr(val, "__float__") else val
            print(fmt % (key, valstr)) if self.verbose else None
            vals.append(val)
        print("-" * n_slashes, flush=True) if self.verbose else None
